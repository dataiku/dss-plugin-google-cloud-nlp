...
for batch in run_by_batch(classify, input_df, text_column, batch_size=BATCH_SIZE, parallelism=PARALLELISM):
    ...
    if output_multiple_categories:
        input_df.set_value(j, categories_column, json.dumps(
            output['categories']) if len(output['categories']) else '')
    else:
        input_df.set_value(j, category_column, output['categories'][0] if len(
            output['categories']) else '')
